#!/usr/bin/env node
/**
 * Build BPE training corpus v3 — maximum sentence diversity for PUA pair coverage.
 *
 * Problem: BPE needs to see PUA_symbol_A followed by PUA_symbol_B many times
 * to learn the merge. Different sentences with different word ORDER create
 * different PUA pairs, giving BPE more merge opportunities.
 *
 * Strategy:
 * - 500+ unique sentence templates with many word combinations
 * - Each template produces a different PUA pair sequence
 * - 20 copies each = ~10k sentences covering thousands of unique PUA pairs
 */
import { writeFileSync, mkdirSync } from 'fs';
import { encode } from '../src/encoder.js';

// ── Massive diverse sentence generation ──
const nouns = [
  ...
  'request', 'push', 'fetch', 'query', 'index', 'schema', 'table', 'column', 'row', 'record',
  'document', 'page', 'site', 'app', 'tool', 'script', 'program', 'application', 'platform', 'server',
  ...
  'chart', 'graph', 'table', 'list', 'grid', 'card', 'tile', 'row', 'cell', 'block',
  'project', 'product', 'feature', 'story', 'ticket', 'issue', 'bug', 'fix', 'patch', 'update',
  'plan', 'design', 'architecture', 'pattern', 'structure', 'framework', 'library', 'package', 'module', 'component',
  'algorithm', 'method', 'technique', 'approach', 'strategy', 'solution', 'implementation', 'optimization', 'improvement', 'enhancement',
  ...
  'company', 'team', 'department', 'group', 'organization', 'enterprise', 'startup', 'business', 'corporation', 'enterprise',
  'customer', 'client', 'partner', 'vendor', 'supplier', 'developer', 'engineer', 'manager', 'director', 'lead',
  ...
  'morning', 'report', 'summary', 'analysis', 'review', 'update', 'status', 'progress', 'result', 'outcome',
  'report', 'document', 'specification', 'requirement', 'constraint', 'limitation', 'assumption', 'dependency', 'risk', 'issue',
  'problem', 'challenge', 'obstacle', 'barrier', 'difficulty', 'complexity', 'uncertainty', 'ambiguity', 'risk', 'threat',
  'solution', 'answer', 'response', 'reaction', 'action', 'step', 'measure', 'action', 'initiative', 'effort',
  'goal', 'objective', 'target', 'milestone', 'deliverable', 'output', 'result', 'outcome', 'impact', 'value',
  'benefit', 'advantage', 'strength', 'opportunity', 'potential', 'capability', 'capacity', 'resource', 'asset', 'tool',
];
...
const verbs = [
  ...
  'connects', 'disconnects', 'binds', 'unbinds', 'listsens', 'accepts', 'rejects', 'drops', 'retries', 'times',
  'configures', 'initializes', 'setup', 'teardown', 'installs', 'uninstalls', 'registers', 'unregisters', 'enables', 'disables',
  'allocates', 'frees', 'releases', 'claims', 'locks', 'unlocks', 'syncs', 'async', 'awaits', 'yields',
];

const adjs = [
  'large', 'small', 'fast', 'slow', 'new', 'old', 'good', 'bad', 'high', 'low',
  'deep', 'wide', 'full', 'empty', 'active', 'inactive', 'valid', 'invalid', 'simple', 'complex',
  'quick', 'immediate', 'instant', 'real', 'live', 'dry', 'wet', 'hot', 'cold', 'warm',
  ...
  'advanced', 'professional', 'enterprise', 'personal', 'internal', 'external', 'system', 'user', 'global', 'regional',
  'single', 'multiple', 'double', 'triple', 'primary', 'secondary', 'tertiary', 'main', 'minor', 'major',
  'critical', 'important', 'significant', 'trivial', 'minor', 'major', 'huge', 'tiny', 'massive', 'microscopic',
];
...
function generateSentences() {
  ...
  // Pattern 1: simple SVO (most common in English)
  for (let i = 0; i < 200; i++) {
    sentences.add(`${pick(articles)} ${pick(nouns)} ${pick(verbs)} ${pick(articles)} ${pick(nouns)}`);
  }

  // Pattern 2: SVO with adjective
  for (let i = 0; i < 200; i++) {
    sentences.add(`${pick(articles)} ${pick(adjs)} ${pick(nouns)} ${pick(verbs)} ${pick(adjs)} ${pick(nouns)}`);
  }

  // Pattern 3: complex sentence with conjunction
  for (let i = 0; i < 150; i++) {
    sentences.add(`${pick(articles)} ${pick(nouns)} ${pick(verbs)} ${pick(nouns)} ${pick(conjunctions)} ${pick(articles)} ${pick(nouns)} ${pick(verbs)} ${pick(nouns)}`);
  }

  // Pattern 4: prepositional phrase
  for (let i = 0; i < 150; i++) {
    sentences.add(`${pick(articles)} ${pick(nouns)} ${pick(verbs)} ${pick(nouns)} ${pick(prepositions)} ${pick(articles)} ${pick(adjs)} ${pick(nouns)}`);
  }

  // Pattern 5: passive voice
  for (let i = 0; i < 100; i++) {
    sentences.add(`${pick(articles)} ${pick(nouns)} was ${pick(verbs)} by ${pick(articles)} ${pick(nouns)}`);
  }

  // Pattern 6: question
  for (let i = 0; i < 100; i++) {
    sentences.add(`does ${pick(articles)} ${pick(nouns)} ${pick(verbs)} ${pick(nouns)}`);
  }

  // Pattern 7: technical compound sentences
  for (let i = 0; i < 200; i++) {
    sentences.add(`the ${pick(nouns)} ${pick(verbs)} ${pick(articles)} ${pick(adjs)} ${pick(nouns)} ${pick(prepositions)} ${pick(articles)} ${pick(nouns)}`);
  }

  // Pattern 8: with adverbs
  const adverbs = ['quickly', 'slowly', 'efficiently', 'correctly', 'properly', 'immediately', 'currently', 'frequently', 'occasionally', 'automatically'];
  for (let i = 0; i < 100; i++) {
    sentences.add(`${pick(articles)} ${pick(nouns)} ${pick(adverbs)} ${pick(verbs)} ${pick(articles)} ${pick(nouns)}`);
  }

  // Pattern 9: comparison
  for (let i = 0; i < 100; i++) {
    sentences.add(`${pick(articles)} ${pick(adjs)} ${pick(nouns)} is more ${pick(adjs)} than ${pick(articles)} ${pick(nouns)}`);
  }

  // Pattern 10: list-style
  for (let i = 0; i < 100; i++) {
    sentences.add(`${pick(nouns)}, ${pick(nouns)}, and ${pick(nouns)} are ${pick(adjs)} ${pick(nouns)}`);
  }

  return [...sentences];
}

// ── Code sentences (diverse patterns) ──
const codeSentences = [
...
function buildCorpus() {
  ...
  const englishSentences = generateSentences();
  console.log('Generated', englishSentences.length, 'unique English sentences');

  // English: 20 copies
  for (let i = 0; i < 20; i++) {
    for (const s of englishSentences) parts.push(s);
  }

  // Code: 20 copies
  for (let i = 0; i < 20; i++) {
    for (const s of codeSentences) parts.push(s);
  }

  // SQL: 15 copies
  for (let i = 0; i < 15; i++) {
    for (const s of sqlSentences) parts.push(s);
  }

  // Shell: 15 copies
  for (let i = 0; i < 15; i++) {
    for (const s of shellSentences) parts.push(s);
  }

  // API: 15 copies
  for (let i = 0; i < 15; i++) {
    for (const s of apiSentences) parts.push(s);
  }
  ...
  console.log('BPE training corpus v3:');
  ...
  console.log('  dict coverage:', (symbolSet.size / 51000 * 100).toFixed(1) + '%');
  console.log('  benchmark copies: 5');
