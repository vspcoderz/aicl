#!/usr/bin/env python3
"""Evaluation probes shared by the Python sweep and evaluation CLIs.

The values are intentionally kept in a dependency-free module.  They are the
same eight benchmark strings and 50 held-out probes used by the legacy
scripts, so a Python evaluation does not silently use a different data set.
"""
...
HELDOUT_PROBES: dict[str, list[str]] = {
    "english": [
...
HELDOUT: list[tuple[str, str]] = [
    (f"{domain}{index}", text)
    for domain, texts in HELDOUT_PROBES.items()
...
def self_check() -> None:
    assert len(BENCH) == 8
    assert len(HELDOUT_PROBES) == 8
    # The historical file calls this the "50-probe" set, but its checked-in
    # data contains 55 entries (15 English + 10 code + 5 in each other domain).
    # Preserve the data rather than silently dropping five probes.
    assert len(HELDOUT) == 55
    assert all(isinstance(name, str) and isinstance(text, str) for name, text in HELDOUT)
    print("heldout probes self-check: 8 benchmark + 55 held-out probes")
...
def main() -> int:
    ...
    if args.self_check:
        ...
        print(f"{len(BENCH)} benchmark texts; {len(HELDOUT)} held-out probes")
    return 0
