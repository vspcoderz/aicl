// Browser-specific port of the canonical Python runtime in aicl/.
// Keep API-compatible with the Python implementation; this is the checked-in
// artifact used by the static GitHub Pages playground.
/**
...
// Building this index used to dominate tokenization for the shipped ~117k-rule
// vocabulary. Vocabularies are immutable after makeVocab(), so cache by identity.
const pairIndexCache = new WeakMap();

function getPairIndex(merges) {
  const cached = pairIndexCache.get(merges);
  ...
  pairIndexCache.set(merges, pairIndex);
  return pairIndex;
}

function bpeMergeInternal(ids, merges, withSpans) {
  const pairIndex = getPairIndex(merges);
  if (pairIndex.size === 0) {
    return withSpans
      ? { ids: ids.slice(), tokenMap: ids.map((_, i) => i) }
      : ids.slice();
  }

  let seq = ids.slice();
  const slots = withSpans ? ids.map((_, i) => [i]) : null;
  while (true) {
    ...
    const next = [];
    const nextSlots = withSpans ? [] : null;
    for (let i = 0; i < seq.length; i++) {
      if (i < seq.length - 1 && seq[i] === bestRule.a && seq[i + 1] === bestRule.b) {
        next.push(bestRule.mergedId);
        if (withSpans) nextSlots.push([...slots[i], ...slots[i + 1]]);
        i++;
      } else {
        next.push(seq[i]);
        if (withSpans) nextSlots.push(slots[i]);
      }
    }
    if (next.length === seq.length) break; // no progress (shouldn't happen)
    seq = next;
    if (withSpans) slots.splice(0, slots.length, ...nextSlots);
  }

  if (!withSpans) return seq;
  const tokenMap = new Array(ids.length);
  for (let token = 0; token < slots.length; token++) {
    for (const position of slots[token]) tokenMap[position] = token;
  }
  return { ids: seq, tokenMap };
}
...
export function bpeMerge(ids, merges) {
  return bpeMergeInternal(ids, merges, false);
}

/**
 * Tokenize AICL text and map every symbol to its resulting token index.
 * This avoids running the BPE merge loop a second time for UI highlighting.
 */
export function tokenizeWithMap(aiclText, vocab) {
  requireText(aiclText, 'aiclText');
  const ids = codePoints(aiclText).map(cpToId);
  return bpeMergeInternal(ids, vocab.merges, true);
}
