"""Portable corpus smoke test for the canonical Python runtime."""
...
def main() -> None:
    ...
    for name, text in TESTS:
        encoded = aicl.encode(text)
        decoded = aicl.decode(encoded["output"])
        if decoded["output"] != text:
            raise AssertionError(f"round trip failed: {name}")
        total_in += encoded["charsIn"]
        total_out += encoded["charsOut"]
        ratio = encoded["charsIn"] / encoded["charsOut"] if encoded["charsOut"] else 0
        print(f"[✓] {name}: {encoded['charsIn']} -> {encoded['charsOut']} ({ratio:.2f}x)")
    ratio = total_in / total_out if total_out else 0
    print(f"\nTOTAL: {total_in} -> {total_out} ({ratio:.2f}x)")
